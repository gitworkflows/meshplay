...
// runMesheryVersionHealthChecks runs checks regarding meshery version and meshplayctl version
func (hc *HealthChecker) runMesheryVersionHealthChecks() error {
	...
	if err != nil || resp.StatusCode != 200 {
		if hc.Options.PrintLogs { // log if we're supposed to
			log.Info("!! failed to check Meshery Server version. try starting Meshery with `meshplayctl system start`")
			skipServerLogs = true
		} else {
			return err
		}
