...
var (
	cmdType     string
	contextdocs string = "See https://docs.meshery.io/reference/meshplayctl/system/context for usage details."
...
func FormatErrorReference() string {
	baseURL := "https://docs.meshery.io/reference/meshplayctl/system"
	switch cmdType {
	case "channel":
...
func ErrResetMeshconfig(err error) error {
	return errors.New(
		...
		[]string{"Verify Meshery server config file is reset to default settings by executing `meshplayctl system context view`" + FormatErrorReference()})
}
...
func ErrRunPortForward(err error) error {
	return errors.New(
		...
		[]string{"Meshery pod is not in running phase", "meshplayctl can't connect to kubernetes with client-go"},
...
func ErrFailedGetEphemeralPort(err error) error {
	return errors.New(
		...
		[]string{"Ensure your Meshery Server is running,", "Ensure meshplayctl is connected to kubernetes cluster with `meshplayctl system check`"},
	)
}
...
func ErrProviderInfo(err error) error {
	return errors.New(
		...
		[]string{"Start Meshery to verify provider. Run `meshplayctl system provider set [provider] --force` to force set the provider" + FormatErrorReference()})
}
